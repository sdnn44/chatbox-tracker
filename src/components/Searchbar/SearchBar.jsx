...
export const SearchBar = () => {
  ...
  const [isLoadingChatter, setIsLoadingChatterisLoadingChatter] =
    useState(true);
  ...
  const { chatboxUsernames } = useGlobalContext();
  ...
  const searchChatter = async () => {
    ...
    const response = {
      data: chatboxUsernames.filter((chatter) =>
        chatter.toLowerCase().includes(searchQuery.toLowerCase())
      ),
    };
  ...
  return (
    <motion.div
      ...
      style={{
        height: isExpanded ? "15rem" : "2.2rem",
    ...
    >
      ...
      {isExpanded && (
        <span className="flex min-w-full min-h-[2px] bg-[#6d3a77]" />
      )}
      {isExpanded && (
        <div className="flex flex-col h-full w-full overflow-hidden overflow-y-auto scrollbar-style">
          {/* <div className="flex border-2 w-full h-full items-center justify-center"> */}
          {isLoadingChatter && isExpanded && <Loader />}
          {!isLoadingChatter && isEmpty && !nochatters && (
            <span className="flex h-full justify-center items-center text-sm opacity-60">
              Zacznij pisać by wyszukać użytkownika
            </span>
          )}
          {!isLoadingChatter && nochatters && (
            <span className="flex h-full justify-center items-center text-sm opacity-60">
          ...
          {!isLoadingChatter &&
            !isEmpty &&
            chatterList.map((item, index) => (
              <ChatterCard key={index} chatter={item} />
            ))}
        </div>
      )}
    </motion.div>
